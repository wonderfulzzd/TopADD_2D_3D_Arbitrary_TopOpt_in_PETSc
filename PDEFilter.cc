...
PDEFilt::PDEFilt (DM da_nodes, PetscScalar rmin) {
  ...
  PetscInt stencilwidth = 1;
    ...
    DMDAStencilType stype;
    {
        ...
        PetscInt M, N, md, nd;
        DMDAGetInfo(da_nodes, NULL, &M, &N, NULL, &md, &nd, NULL, NULL, NULL, &bx, &by, NULL, &stype);
    ...
    DMDACreate2d(PETSC_COMM_WORLD, bx, by, stype, nn[0] - 1, nn[1] - 1, md, nd, 1, overlap, Lx, Ly,
                 &(da_element));
    // Initialize
  ...
  // coincide
  PetscInt md, nd, pd;
  DMDAGetInfo (da_nodal, NULL, NULL, NULL, NULL, &md, &nd, &pd, NULL, NULL,
  NULL, NULL, NULL, NULL);
  PetscInt *Lx = new PetscInt[md];
  PetscInt *Ly = new PetscInt[nd];
  ...
  PetscInt overlap = 0;
  // Create the element grid:
  DMDACreate3d (PETSC_COMM_WORLD, bx, by, bz, stype, nn[0] - 1, nn[1] - 1,
      nn[2] - 1, md, nd, pd, 1, overlap, Lx, Ly, Lz, &(da_element));
  // Initialize
  DMSetFromOptions (da_element);
  DMSetUp (da_element);
  ...
  // Create T matrix
  {
    ...
    MatCreateAIJ (PETSC_COMM_WORLD, m, n, PETSC_DETERMINE, PETSC_DETERMINE, 8,
    NULL, 7, NULL, &T);
...
PetscErrorCode PDEFilt::FilterProject (Vec OX, Vec FX) {
  ...
  PetscPrintf (PETSC_COMM_WORLD,
      "PDEFilter solver:  iter: %i, rerr.: %e, time: %f\n", niter, rnorm,
      t2 - t1);
  return ierr;
}
  ...
  if (pcmg_flag) {
    ...
    // AVOID THE DEFAULT FOR THE MG PART
    {
      ...
      for (PetscInt k = 1; k < nlvls; k++) {
        ...
        PETSC_DEFAULT, restart); // NOTE maxitr=restart;
...
void PDEFilt::PDEFilterMatrix_2D(PetscScalar dx, PetscScalar dy, PetscScalar RR, PetscScalar* KK,
                                 PetscScalar* T) {
    PetscScalar t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15,
    ...
    t6 = t2 * t4;  // r^2 * dy^2
    t7 = t3 * t4;  // dx^2 * dy^2
    t8 = t1 * (3 * t5 + 3 * t6 + t7) / 9.0;
    t9 = t1 * (3 * t5 - 6 * t6 + t7) / 18.0;
    t10 = t1 * (-6 * t5 - 6 * t6 + t7) /36.0;
    t11 = t1 * (-6 * t5 + 3 * t6 + t7) / 18.0;
    t12 = t9;
    t13 = t8;
    t14 = t11;
    t15 = t10;
...
#elif DIM == 3
PetscErrorCode PDEFilt::DMDAGetElements_3D (DM dm, PetscInt *nel, PetscInt *nen,
    const PetscInt *e[]) {
  DM_DA *da = (DM_DA*) dm->data;
  PetscInt i, xs, xe, Xs, Xe;
  PetscInt j, ys, ye, Ys, Ye;
...
void PDEFilt::PDEFilterMatrix (PetscScalar dx, PetscScalar dy, PetscScalar dz,
  ...
  PetscScalar t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t15, t16, t18,
      t22, t23, t27, t28, t32, t36, t37, t41, t45, t49, t53;
  t3 = 1.0 / dx / dy;
  t4 = 1 / dz;
  t5 = RR * RR;
  t6 = dx * dx;
